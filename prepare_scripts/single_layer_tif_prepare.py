...
def prep_dataset(nbar_path, prep_file, processing_level, product_type, platform_code, instrument, bands):
    ...
    doc = {
        ...
        'platform': {'code': str(platform_code)},
        'instrument': {'name': str(instrument)},
        'extent': {
            ...
            'coord': {
                'll': {'lat': coord_lower_left[1], 'lon': coord_lower_left[0]},
                'lr': {'lat': coord_lower_right[1], 'lon': coord_lower_right[0]},
                'ul': {'lat': coord_upper_left[1], 'lon': coord_upper_left[0]},
                'ur': {'lat': coord_upper_right[1], 'lon': coord_upper_right[0]},
            }
        },
        'format': {'name': 'GeoTiff'},
        'grid_spatial': {
            'projection': {
                'spatial_reference': str(src.crs_wkt),
                'geo_ref_points': {
                    'ul': {'x': left, 'y': top},
                    'ur': {'x': right, 'y': top},
                    'll': {'x': left, 'y': bottom},
        ...
        'image': {
            'satellite_ref_point_start': {'x': prep_file.split("_")[1], 'y': prep_file.split("_")[2]},
            'satellite_ref_point_end': {'x': prep_file.split("_")[1], 'y': prep_file.split("_")[2]},
            'bands': images
        },
...
@click.command(help="Prepare Layered GeoTIFF ingestion into the Data Cube.")
...
@click.option('-b', '--bands', multiple=True,  help='Pass in a band number and the associated name (ex. -b 1=swir -b 2=red -b 3=blue ...)')
def main(datasets, processing_level, product_type, platform_code, instrument, bands):
    arg_product_type = ''
    arg_platform_code = ''
    arg_instrument = '' 

    arg_processing_level = str(processing_level) if processing_level is not None else '0'
    arg_product_type = str(product_type) if product_type is not None else None
    arg_platform_code = str(platform_code) if platform_code is not None else None
    arg_instrument = str(instrument) if instrument is not None else None
    
    if arg_product_type is None or arg_platform_code is None:
        print("Product Type and Platform Code must be included to run the script. Please include these parameters and run the script again.")
        sys.exit(2)
